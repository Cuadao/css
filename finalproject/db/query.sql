...
CREATE TABLE `alceneptunobox`.`order_state` (
  `id_state` INT NOT NULL AUTO_INCREMENT,
  `state_name` VARCHAR(45) NOT NULL,
  `state_descr` VARCHAR(60) NOT NULL,
  PRIMARY KEY (`id_state`));
  
INSERT INTO `alceneptunobox`.`order_state` (`id_state`, `state_name`, `state_descr`) VALUES 
...
CREATE TABLE `alceneptunobox`.`orders` (
  ...
  CONSTRAINT `id_customer`
    ...
    REFERENCES `alceneptunobox`.`customers` (`id_cust`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `state_order`
    FOREIGN KEY (`state_order`)
    REFERENCES `alceneptunobox`.`order_state` (`id_state`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `id_product`
    FOREIGN KEY (`id_product`)
    REFERENCES `alceneptunobox`.`products` (`id_prod`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `create_by`
    FOREIGN KEY (`create_by`)
    REFERENCES `alceneptunobox`.`users` (`id_user`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION);
    
INSERT INTO `alceneptunobox`.`orders` (`id_order`, `order_date`, `id_customer`, `state_order`,
 `create_by`, `quatity`, `id_product`, `total`, `observations`) VALUES 
('1', '01012020', '1', '1', '1', '2', '1', '50', 'A'),
('2', '01012020', '2', '1', '1', '3', '2', '100', 'B');
